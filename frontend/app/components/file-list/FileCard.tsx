...
function ScoreItem({
    ...
    const ratio = !placeholder && maxScore > 0 ? score / maxScore : 0;
    const barWidth = Math.min(ratio * 100, 100);
    const colorClass = placeholder
        ? "text-slate-400"
        : ratio >= 0.75
          ? "text-green-600"
          : ratio >= 0.5
    ...
    return (
        <div className="flex flex-col w-[64px]">
            <span
                className="text-[10px] text-slate-500 font-medium mb-1 truncate"
                title={label}
            >
                {label}
            </span>
            <div className="flex items-center gap-1.5">
                ...
                <span
                    className={`${placeholder ? "text-xl leading-none" : "text-[12px]"} font-semibold tabular-nums ${colorClass}`}
                >
                    {display}
                </span>
            </div>
...
export function FileCard({
    ...
    const scoreLabel = scoreMode === "average" ? "Avg" : "Total";
